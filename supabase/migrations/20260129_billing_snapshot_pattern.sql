-- 💰 Billing Snapshot Pattern Migration
-- Store calculated GST and totals at order time for historical accuracy
-- Supports product-wise GST: 5%, 18%, or none
...
-- Assumes current unit_price contains GST and we need to extract base
UPDATE order_items
SET 
  -- For existing orders without GST info, assume they were created with old 8% tax
  -- We'll mark them as 0% GST and keep prices as-is (historical data preservation)
  base_price = COALESCE(base_price, unit_price / 100.0),
  gst_rate = COALESCE(gst_rate, 0),  -- Mark as no GST for old orders
...
ALTER TABLE orders
  ...
  ADD COLUMN IF NOT EXISTS grand_total DECIMAL(10,2);          -- Final total (subtotal + total_gst + shipping)
...
SET
  ...
  grand_total = COALESCE(grand_total, amount)
WHERE
  subtotal IS NULL;
...
CREATE INDEX IF NOT EXISTS idx_orders_grand_total 
...
ALTER TABLE orders
  ADD CONSTRAINT check_order_totals_non_negative 
    CHECK (subtotal >= 0 AND total_gst >= 0 AND grand_total >= 0);
...
-- ✅ Migration complete!
-- Billing snapshots now store:
-- - Product-wise GST (5%, 18%, or none)
-- - Base prices and GST amounts
